...
# הגדרת מצב פיתוח בסרגל הצד
is_dev_mode = st.sidebar.checkbox("מצב פיתוח", key="dev_mode", value=False)

if is_dev_mode:
    st.sidebar.markdown("### כלי פיתוח")
    
    # אפשרויות למעבר מהיר בין שלבים
    dev_stage = st.sidebar.radio(
        "מעבר ישיר לשלב:",
        ["welcome", "context", "image", "eval", "end", "part2_start", "part2_questions", "part2_end"],
        index=0
    )
    
    if st.sidebar.button("עבור לשלב שנבחר"):
        st.session_state.stage = dev_stage
        if dev_stage == "part2_questions":
            if "part2_graph_index" not in st.session_state:
...
# אפשרות לדלג על גרפים אם במצב פיתוח וכבר התחלנו בניסוי
if is_dev_mode and "graph_index" in st.session_state and "filtered_df" in st.session_state:
    st.sidebar.markdown(f"### גרף נוכחי: {st.session_state.graph_index+1}/{len(st.session_state.filtered_df)}")
    
    new_graph_index = st.sidebar.number_input(
        "דלג לגרף מספר:",
        min_value=1,
        max_value=len(st.session_state.filtered_df),
        value=st.session_state.graph_index+1
    )
    
    if st.sidebar.button("דלג לגרף שנבחר"):
        st.session_state.graph_index = new_graph_index - 1
        if st.session_state.stage in ["context", "image", "eval"]:
            st.session_state.stage = "context"
        st.rerun()

# אפשרות לבחירת וריאציה אם במצב פיתוח
if is_dev_mode and "variation" in st.session_state:
    new_variation = st.sidebar.selectbox(
        "שנה וריאציה:",
        ["V1", "V2", "V3", "V4"],
        index=["V1", "V2", "V3", "V4"].index(st.session_state.variation)
    )
    
    if new_variation != st.session_state.variation and st.sidebar.button("החל וריאציה"):
        st.session_state.variation = new_variation
        st.session_state.filtered_df = df[df[st.session_state.variation] == 1].reset_index(drop=True)
        if st.session_state.filtered_df.empty:
            st.sidebar.error(f"אין נתונים בתנאי {st.session_state.variation}")
        else:
            st.sidebar.success(f"וריאציה שונתה ל-{new_variation}")
            st.session_state.graph_index = 0
            st.session_state.stage = "welcome"
        st.rerun()
